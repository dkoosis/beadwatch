...
// writeNorthStar drops a NORTH_STAR.md into a fresh repo dir and returns the
// dir — Roadmap's legacy-fallback tests need a real file to resolve.
...
// TestRoadmapReadsROADMAPEpicsSection is the sdlc-standard format: the Go twin
// of roadmap-epics.sh's roadmap_parse — `N. [status] <title> → <id>[, <id>...]`
...
// ROADMAP.md, pinned in bead-journey.test.sh's parse_epics case): a title too
// long for one line wraps onto an indented continuation carrying the arrow and
// ids. The continuation must fold into its numbered line before parsing, not
// be mis-read as a separate, arrow-less numbered-list item.
...
// TestRoadmapPrefersROADMAPFileOverNorthStar: when both files exist,
// ROADMAP.md wins — it is the sdlc standard; NORTH_STAR.md is the legacy
// pointer, resolved only when ROADMAP.md is absent.
...
// original `## roadmap` + id-first-token parse is the final fallback,
// unchanged (TestRoadmapReadsOrderedEpicIDs already pins that format's own
// edge cases; this just proves the fallback actually fires from Roadmap's new
// resolution order).
