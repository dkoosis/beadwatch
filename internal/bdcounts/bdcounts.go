...
// agent com.trixi.beadwatch writes (scripts/tool-install-launchd.sh authors the plist).
...
// `beadwatch` at — the ONE source of that number on the reader side, mirroring the
// plist's StartInterval (scripts/tool-install-launchd.sh, <integer>120). Staleness is
// judged against 2× this: a single missed tick is normal jitter, two is a refresher
// that stopped.
//
// History: the plist was authored by cc-plugins' bd-counts-install.sh (bw-4id).
